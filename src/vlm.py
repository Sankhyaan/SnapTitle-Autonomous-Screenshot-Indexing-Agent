...
def generate_caption_from_image(
    ...
    for attempt in range(1, max(1, max_retries) + 1):
        try:
            ...
            logger.info(f"Sending image '{image_path.name}' to VLM ({target_model}) [attempt {attempt}]...")
            ...
            logger.warning(f"VLM captioning attempt {attempt}/{max_retries} failed for '{image_path.name}' using model '{model}': {err}")
